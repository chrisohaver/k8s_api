...
func TestDnsProgrammingLatency(t *testing.T) {
	...
	k := new(Kubernetes)
	dnsCon := new(dnsControl)
	k.APIConn = dnsCon
	k.opts.skipAPIObjectsCleanup = true
	...
	informerFuncs := k.Informers()
	epInformer := informerFuncs["endpoints"](ctx, client)
	svcInformer := informerFuncs["service"](ctx, client)

	dnsCon.epLister = epInformer.Lister.(cache.Indexer)
	dnsCon.svcLister = svcInformer.Lister.(cache.Indexer)
	...
	go epInformer.Controller.Run(stop)
	go svcInformer.Controller.Run(stop)
	...
	createService(t, client, dnsCon, "my-service", api.ClusterIPNone)
	...
	createService(t, client, dnsCon, "clusterIP-service", "10.40.0.12")
	createEndpoints(t, client, "clusterIP-service", now.Add(-8*time.Second), nil)

	createService(t, client, dnsCon, "headless-no-annotation", api.ClusterIPNone)
	createEndpoints(t, client, "headless-no-annotation", nil, nil)

	createService(t, client, dnsCon, "headless-wrong-annotation", api.ClusterIPNone)
